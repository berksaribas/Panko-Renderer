...
void Deferred::init_images(EngineData& engineData, VkExtent2D imageSize)
{
    ...
    _deferredColorImage = vkutils::create_image(
        &engineData, COLOR_32_FORMAT,
        VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, extent3D);

    _deferredColorImageBinding = engineData.renderGraph->register_image_view(
        &_deferredColorImage,
        {.sampler = Vrg::Sampler::LINEAR, .baseMipLevel = 0, .mipLevelCount = 1},
        "DeferredColorImage");
}

void Deferred::render(EngineData& engineData, SceneData& sceneData, GBuffer& gbuffer,
