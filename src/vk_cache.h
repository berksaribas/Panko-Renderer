        ...
        bool operator==(DescriptorSetCache const& rhs) const noexcept {
            if (descriptorSets.size() == rhs.descriptorSets.size()) {
                for (int i = 0; i < descriptorSets.size(); i++) {
                    if (descriptorSets[i].type != rhs.descriptorSets[i].type || descriptorSets[i].imageLayout != rhs.descriptorSets[i].imageLayout || descriptorSets[i].imageView != rhs.descriptorSets[i].imageView || descriptorSets[i].buffer != rhs.descriptorSets[i].buffer || descriptorSets[i].image != rhs.descriptorSets[i].image) {
                        return false;
                    }
                }
            }
    ...
    struct DescriptorSetLayoutCache {
        ...
        bool operator==(DescriptorSetLayoutCache const& rhs) const noexcept {
            if (descriptorTypes.size() == rhs.descriptorTypes.size()) {
                return memcmp(descriptorTypes.data(), rhs.descriptorTypes.data(), sizeof(VkDescriptorType) * descriptorTypes.size()) == 0;
            }
            return false;
        }
    };
        ...
        bool operator==(PipelineLayoutCache const& rhs) const noexcept {
            ...
            if (pushRanges.size() == rhs.pushRanges.size()) {
                for (int i = 0; i < pushRanges.size(); i++) {
                    if (pushRanges[i].offset != rhs.pushRanges[i].offset || pushRanges[i].size != rhs.pushRanges[i].size || pushRanges[i].stageFlags != rhs.pushRanges[i].stageFlags) {
                        return false;
                    }
                }
            }
    ...
    struct DescriptorSetCache_hash {
        size_t operator()(DescriptorSetCache const& x) const noexcept {
            ...
            for (int i = 0; i < x.descriptorSets.size(); i++) {
                hash_combine(h, x.descriptorSets[i].type, x.descriptorSets[i].imageLayout, x.descriptorSets[i].imageView.baseMipLevel, x.descriptorSets[i].imageView.mipLevelCount, x.descriptorSets[i].imageView.sampler, reinterpret_cast<uint64_t>(x.descriptorSets[i].buffer), reinterpret_cast<uint64_t>(x.descriptorSets[i].image));
            }
            return h;
        }
    };
    ...
    struct PipelineLayoutCache_hash {
        size_t operator()(PipelineLayoutCache const& x) const noexcept {
            ...
            for (int i = 0; i < x.pushRanges.size(); i++) {
                hash_combine(h, x.pushRanges[i].offset, x.pushRanges[i].size, x.pushRanges[i].stageFlags);
            }
            return h;
        }
    };
