...
void GBuffer::init_images(EngineData& engineData, VkExtent2D imageSize)
{
	...
	for(int i = 0; i < 2; i++)
	{
		...
		_gbufferdata[i].gbufferUVImage = vkutils::create_image(&engineData, COLOR_16_FORMAT, VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, extent3D);

		_gbufferdata[i].gbufferDepthImage = vkutils::create_image(&engineData, DEPTH_32_FORMAT, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, extent3D);

		auto index = std::to_string(i);

		_gbufferdata[i].albedoMetallicBinding = engineData.renderGraph->register_image_view(&_gbufferdata[i].gbufferAlbedoMetallicImage, {
		...
		_gbufferdata[i].uvBinding = engineData.renderGraph->register_image_view(&_gbufferdata[i].gbufferUVImage, {
			.sampler = Vrg::Sampler::NEAREST,
			.baseMipLevel = 0,
			.mipLevelCount = 1
		}, "GbufferUVImage" + index);
		_gbufferdata[i].depthBinding = engineData.renderGraph->register_image_view(&_gbufferdata[i].gbufferDepthImage, {
			.sampler = Vrg::Sampler::NEAREST,
			.baseMipLevel = 0,
			.mipLevelCount = 1
...
void GBuffer::render(EngineData& engineData, SceneData& sceneData, std::function<void(VkCommandBuffer cmd)>&& function)
{
	...
	VkClearValue materialGbufferColor = {
